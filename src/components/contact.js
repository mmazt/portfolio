...
class contact extends React.Component {
  ...
  render(){
    ...
    const responsive_pad1 = window.innerHeight >= 1000 ? '12%' : '3%'
    const responsive_pad2 = window.innerHeight >= 1000 ? '12%' : '1,5%'
    return(
      ...
      {this.state.nuvens_pequenas.map((nuvem, i) => <Nuvem key={i} x={nuvem.x} y={nuvem.y} size={nuvem.size} classe='nuvem_pequena'/>)}
        ...
        <Grid centered style={{ paddingTop: responsive_pad1}} padded>
          <Grid.Row style={{textAlign: 'center', zIndex: '1000'}}>
            <Header as='h1'  className='texto-basico'>Contato</Header>
          </Grid.Row>
          <Grid.Row style={{ paddingTop:'10px', zIndex: '1000'}}>
          ...
          <Grid.Row only='tablet computer' style={{ position:'relative', paddingTop: responsive_pad2}}>
